...
| 200 | path `/api/*` AND header `X-Product-Id=protoapp` | `ecs-target-group` |
...
1. `variables.tf`: change `product`, `domain_name`, `cloudflare_zone_id`, `alb_rule_priority` (must be unique — sjocamp=100, protoapp=200; new projects from 300 in steps of 10)
