...
Every product rule matches on the header — there is no header-less catch-all. An
`/api/*` request arriving without a recognised `X-Product-Id` gets the 404 rather
than falling through to whichever product happens to hold the lowest priority.
The rules are generated by `modules/product/alb-routing.tf`; priorities live in
each product's `alb_rule_priority`.

## What was done in this PR (already applied)

> Historical record. `products/protoapp/` below refers to the directory as it was
> then; it is now `products/meerkat/` and its SSM paths are `/meerkat/*`.
...
- Lowered protoapp's ALB listener rule priority from 1 → 1000 to make room for sjocamp at priority 100. (Superseded: the later `modules/product` refactor moved protoapp to priority 200 and gave it a header condition. See the routing table above.)
...
1. `variables.tf`: change `product`, `domain_name`, `cloudflare_zone_id`, `alb_rule_priority` (must be unique — sjocamp=100, meerkat=200; new projects from 300 in steps of 10)
